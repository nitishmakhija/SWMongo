...
public final class Cursor<T> {
    ...
    internal convenience init?(namespace: String, collection: Collection, connection: Connection, reply: ServerReply, chunkSize: Int32, transform: @escaping Transformer) throws {
        self.init(namespace: namespace, collection: collection, connection: connection, cursorID: reply.cursorID, initialData: try reply.documents.compactMap(transform), chunkSize: chunkSize, transform: transform)
    }
    ...
    internal convenience init(cursorDocument cursor: Document, collection: Collection, connection: Connection, chunkSize: Int32, transform: @escaping Transformer) throws {
        ...
        self.init(namespace: namespace, collection: collection, connection: connection, cursorID: cursorID, initialData: try firstBatch.arrayRepresentation.compactMap{ Document($0) }.compactMap(transform), chunkSize: chunkSize, transform: transform)
    }
    ...
    internal init<B>(base: Cursor<B>, transform: @escaping (B) throws -> (T?)) throws {
        ...
        self.data = try base.data.compactMap(transform)
        ...
        return async(timeoutAfter: .seconds(60)) {
            do {
                if self.collection.database.server.serverData?.maxWireVersion ?? 0 >= 4 {
                    ...
                    let results = try reply.documents.compactMap(self.transform)
