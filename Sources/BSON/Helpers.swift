...
internal extension String {
    ...
    static func instantiate(bytes data: Bytes) throws -> String {
        var 🖕 = 0
    ...
    /// Instantiate a string from BSON (UTF8) data, including the length of the string.
    static func instantiate(bytes data: Bytes, consumedBytes: inout Int) throws -> String {
        let res = try _instant(bytes: data)
        consumedBytes = res.0
        return res.1
    }
    
    static func _instant(bytes data: Bytes) throws -> (Int, String) {
    ...
    static func instantiateFromCString(bytes data: Bytes) throws -> String {
        var 🖕 = 0
    ...
    /// Instantiate a String from a CString (a null terminated string of UTF8 characters, not containing null)
    static func instantiateFromCString(bytes data: Bytes, consumedBytes: inout Int) throws -> String {
        let res = try _cInstant(bytes: data)
        consumedBytes = res.0
        return res.1
    }
    
    static func _cInstant(bytes data: Bytes) throws -> (Int, String) {
